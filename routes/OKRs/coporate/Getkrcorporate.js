...
// Get KR Team
Router.get('/:ID_Corporate', async (req, res, next) => {
    var ID_Corporate = req.params.ID_Corporate;
    let arrKRCorporate = []
    ...
    function getokrteam(word, callback) {
        ...
        var wheres =' Where ID_Corporate = ?'
        var sqls = selects+froms+wheres;
        mysqlConnection.query(sqls,[ID_Corporate], function(err, rowss) {
    ...
    function getteamname(word, callback) {
        ...
        var wheres =' Where personals.ID_Corporate = ?'
        var sqls = selects+froms+wheres;
        mysqlConnection.query(sqls,[ID_Corporate], function(err, rowss) {
            if(err) return callback(err);
            callback(null, rowss);
    ...
    function getteam(word, callback) {
        getteamnamecallback((err,Team_Name)=>{
            ...
            var wheres =' Where teams.ID_Corporate = ?'
            var sqls = selects+froms+wheres;
            mysqlConnection.query(sqls,[ID_Corporate], function(err, rowss) {
                if(err) return callback(err);
                callback(null, rowss);
